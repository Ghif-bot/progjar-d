...
def send_command(command_str, is_secure=False):
    ...
    if is_secure == True:
        sock = make_secure_socket(addressing_server, port_server)
    else:
...
def get_data_player(nomor=0, is_secure=False):
    cmd=f"get_data_player {nomor}\r\n\r\n"
    result = send_command(cmd, is_secure=is_secure)
    if (result):
        pass
    else:
...
def see_version(is_secure=False):
    cmd=f"versi \r\n\r\n"
    result = send_command(cmd, is_secure=is_secure)
    return result

def get_data_player_multithread(total_request, porportion_data):
    total_response = 0
    texec = dict()
    temp1 = datetime.datetime.now()

    for k in range(total_request):
        # bagian ini merupakan bagian yang mengistruksikan eksekusi get_data_player secara multithread
        texec[k] = threading.Thread(
            target=get_data_player, args=(random.randint(1, 20), True))
        texec[k].start()
    ...
    porportion_data.append([total_request, total_request, total_response, complete])

if __name__ == '__main__':
    h = see_version(is_secure=True)
    if (h):
        print(h)
    ...
    for request in total_request:
        get_data_player_multithread(request, porportion_data)

    porportion_header = ["Thread Count", "Request Count", "Response Count", "Execution Time", "Average Latency"]
    print(tabulate( porportion_data, headers=porportion_header, tablefmt="fancy_grid"))
